...
/* =============================================================
   BookingForm — multi-section, validating, package-aware
   ============================================================= */
const SESSIONS = [
...
function BookingForm({ pickedSession, onPickSession }) {
  const [data, setData] = useState({
    name: '',
    email: '',
    org: '',
    role: '',
    session: pickedSession || 'discovery',
    topic: TOPICS[0],
    challenge: '',
  ...
  // progress: how many of the required fields are filled
  ...
  if (submitted) {
    return (
      <div className="form-success">
        ...
        <p>
          A confirmation has been queued to <b>{data.email}</b>. Expect a
          reply within 48 hours, on weekdays. If you booked a Strategy
          Session, an invoice and calendar invite follow in the same
          thread.
        </p>
        <div className="receipt">
          ...
          <div className="row"><span>Topic</span><span style={{maxWidth:'24ch',textAlign:'right'}}>{data.topic}</span></div>
    ...
    <form className="form-card" onSubmit={submit} noValidate>
      ...
      <div className="field-row">
        <div className={`field ${errors.org ? 'error' : ''}`}>
          ...
          <input
            value={data.org}
            placeholder="Company, ministry, cooperative…"
        ...
        <div className="field">
          ...
          <input
            value={data.role}
            placeholder="Founder, Director, etc. (optional)"
        ...
        <div className="session-pick">
          {SESSIONS.map((s) => (
            <button
              ...
              onClick={() => {
                setField('session', s.id);
                if (onPickSession) onPickSession(s.id);
              }}
            >
      ...
      <div className="form-actions">
        ...
        <button className="btn btn-primary" type="submit">
          Send brief <span className="arrow" />
        </button>
      </div>
    </form>
  );
...
Object.assign(window, { BookingForm, SESSIONS, TOPICS });
